...
public class NewLobbyController : MonoBehaviour
{
	...
	public void resize()
	{
		...
		this.mainBlockSubTitle.transform.GetComponent<TextContainer>().width=mainBlockSize.x-2f*ApplicationDesignRules.blockHorizontalSpacing-ApplicationDesignRules.button62WorldSize.x;
	...
	public void drawResults()
	{
		...
		for(int i =0;i<this.pagination.nbElementsPerPage;i++)
		{
			if(this.pagination.chosenPage*this.pagination.nbElementsPerPage+i<model.lastResults.Count)
			{
                ...
                this.results[i].transform.FindChild("picture").GetComponent<SpriteRenderer>().sprite=BackOfficeController.instance.returnLargeProfilePicture(model.lastResults[this.pagination.chosenPage*this.pagination.nbElementsPerPage+i].Opponent.IdProfilePicture);
