...
public class NewFocusedCardMarketController : NewFocusedCardController
{
	...
	public override void updateFocusFeatures()
	{
		...
		if(this.c.Price>ApplicationModel.credits && this.c.onSale==1)
		{
			...
			this.gameObject.transform.FindChild("FocusFeature0").transform.GetComponent<NewFocusedFeaturesMyGameController>().setIsClickable(false);
		}
		else if(this.c.onSale==1)
		{
			...
			this.gameObject.transform.FindChild("FocusFeature0").transform.GetComponent<NewFocusedFeaturesMyGameController>().setIsClickable(true);
		}
		else
		{
