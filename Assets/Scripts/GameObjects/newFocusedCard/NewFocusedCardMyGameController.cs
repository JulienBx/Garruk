...
public class NewFocusedCardMyGameController : NewFocusedCardController
{
	public override void updateFocusFeatures()
	{
		...
		if(this.c.ExperienceLevel!=10)
		{
			...
			this.gameObject.transform.FindChild("FocusFeature1").GetComponent<NewFocusedFeaturesMyGameController>().setIsClickable(true);
		}
		else
		{
			this.gameObject.transform.FindChild("FocusFeature1").FindChild("Title").GetComponent<TextMeshPro>().text="Niveau maximum atteint";
			this.gameObject.transform.FindChild("FocusFeature1").transform.GetComponent<NewFocusedFeaturesMyGameController>().setIsClickable(false);
		}
		...
		if(this.c.Decks.Count>0)
		{
			...
			this.gameObject.transform.FindChild("FocusFeature3").transform.GetComponent<NewFocusedFeaturesMyGameController>().setIsClickable(false);
		...
		else if(this.c.onSale==1)
		{
			...
			this.gameObject.transform.FindChild("FocusFeature3").transform.GetComponent<NewFocusedFeaturesMyGameController>().setIsClickable(true);
		}
		else
		{
