...
public class PiegeAffaiblissant : GameSkill
{
	...
	public override void applyOn(){
		...
		for(int i = 0 ; i < base.tileTargets.Count ; i++){
			...
			GameView.instance.getGC().addTileModifier(target, amount, ModifierType.Type_WeakeningTrap, ModifierStat.Stat_No, -1, 3, "Piège affaiblissant", "Réduit de "+amount+"% l'attaque du héros touché pendant 2 tours", "Permanent. Non visible du joueur adverse");
	...
	public override void activateTrap(int[] targets, int[] args){
		...
		GameView.instance.getGC().addCardModifier(targets[0], -1*amount, ModifierType.Type_BonusMalus, ModifierStat.Stat_Attack, 2, 5, "Parapiège", "Attaque diminuée de "+amount, "Actif 2 tours");
