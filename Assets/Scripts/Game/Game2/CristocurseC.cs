...
public class CristocurseC : SkillC
{
	public CristocurseC(){
		base.id = 41 ;
		base.ciblage = 5;
	...
	public override void resolve(int x, int y, Skill skill){
		...
		List<TileM> neighbours = Game.instance.getBoard().getAnyoneAdjacentCristal(Game.instance.getBoard().getCurrentBoard(), Game.instance.getCurrentCard(), new TileM(x,y));
	...
	public override void effects(int targetID, int level, int z){
		...
		int degats = caster.getDegatsAgainst(target, Mathf.RoundToInt(2+level+(4f*z)/100f));
		...
		for(int i = 0 ; i < neighbours.Count ;i++){
			if(board[neighbours[i].x, neighbours[i].y]>=0){
				if(board[neighbours[i].x, neighbours[i].y]!=Game.instance.getCurrentCardID()){
					...
					tempScore = caster.getDamageScore(target, 2+s.Power, 6+s.Power);
