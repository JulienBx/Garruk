...
public class AuthenticationScript : MonoBehaviour 
{
	...
	{
		GUI.Label(
			new Rect(10, 5, 140, 20), "Page d'authentification");	// label texte
		GUI.Label(
			new Rect(10, 30, 100, 20), "votre identifiant:"); 	
		GUI.Label(
			new Rect(10, 60, 160, 20), "votre mot de passe:");
		
		formNick = GUI.TextField (
			new Rect(150, 30, 100, 20), formNick); 					// Input pour le pseudo
		formPassword = GUI.TextField(
			new Rect(150, 60, 100, 20), formPassword); 				// Celui pour le mot de passe
		
		if ( GUI.Button(
			new Rect(10, 90, 100, 20), "s'authentifier"))			// bouton de connexion
		{ 
